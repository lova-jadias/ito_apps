  ...
  Future<void> _validerPaiement() async {
    ...
    try {
      ...
      final recuData = await Supabase.instance.client.from('recus').insert({
        ...
        'ref_transaction': _refController.text,
        'montant_total': total,
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
            'Paiement enregistré avec succès pour ${_selectedStudent!['nom']}.'),
  ...
  void _showPostPaymentDialog() {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Que voulez-vous faire maintenant ?'),
        actions: [
          TextButton(
            onPressed: () {
            ...
            child: Text('Nouveau paiement (même étudiant)'),
          ),
          TextButton(
            onPressed: () {
              Navigator.of(ctx).pop();
            ...
            child: Text('Voir État Individuel'),
          ),
          TextButton(
            onPressed: () {
              Navigator.of(ctx).pop();
              context.go('/etat-groupe');
            },
            child: Text('Voir État par Groupe (BAG)'),
          ),
          TextButton(
            onPressed: () {
              Navigator.of(ctx).pop();
        ...
        children: [
          ...
          Card(
            ...
            child: ListTile(
              ...
              subtitle: Text(
                  'ID: ${_selectedStudent!['id_etudiant_genere']} | Groupe: ${_selectedStudent!['groupe']} | Statut: ${_selectedStudent!['statut']}'),
            ),
          ),
          SizedBox(height: 16),
          Wrap(
          ...
          Wrap(
            ...
            runSpacing: 16,
            children: [
              _buildDropdown("Motif de Paiement *", config['Motifs'],
                  _updateMoisDeField),
              _buildDropdown("Mode de Paiement *", config['ModePaiement'],
                      (val) => setState(() => _selectedModePaiement = val)),
              ConstrainedBox(
                ...
                child: DropdownButtonFormField<String>(
                  ...
                  items: (config['Mois de'] ?? [])
                      .map((item) =>
                      DropdownMenuItem(value: item, child: Text(item)))
                      .toList(),
                  onChanged: _moisDeActif
                      ? (val) => setState(() => _selectedMois = val)
                      : null,
                  validator: (value) => (_moisDeActif && value == null)
                      ? 'Mois requis pour écolage'
                      : null,
                ),
              ),
              _buildTextField("Nº Reçu *", _recuController),
              _buildTextField("Référence Transaction", _refController,
                  isRequired: false),
              ConstrainedBox(
                constraints: BoxConstraints(minWidth: 280),
                child: TextFormField(
          ...
          SizedBox(height: 16),
          Text(
              "Note: Pour les paiements groupés sur un seul reçu, enregistrez le premier motif, puis recommencez avec le même Nº Reçu.",
